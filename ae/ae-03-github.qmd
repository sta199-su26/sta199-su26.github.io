...
-   Demo: Print a data frame that only contains the variables `dep_delay` and `arr_delay`.
...
-   Demo: Print a data frame that keeps every variable except `dep_delay`.
...
-   Demo: Print a data frame that includes all variables between `year` through `dep_delay` (inclusive). These are all variables that provide information about the departure of each flight.
...
-   Demo: Use the `select` helper `contains()` to print a data frame that includes the variables associated with the arrival, i.e., contains the string `"arr\_"` in the name.
...
-   **Your turn:** Print a data frame that only includes the plane tail number (`tailnum`), carrier (`carrier`), and departure delay for the flight with the longest departure delay. What is the plane tail number (`tailnum`) for this flight?
...
-   **Your turn:** What is the median departure delay for each airport around NYC (`origin`)?
