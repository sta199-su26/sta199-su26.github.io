...
age_gaps <- read_excel("data/age-gaps.xlsx", 
...
Save these subsets as three appropriately named data frames.
...
age_gaps_hetero = age_gaps_hetero |>
...
woman_older <- age_gaps_hetero |>
  filter(older_gender == "woman older")
  
man_older <- age_gaps_hetero |>
    filter(older_gender == "man older")
  
same_age <- age_gaps_hetero |>
    filter(older_gender == "same age")
...
sales_raw <- read_excel("data/sales.xlsx",
                        skip = 4,
                        col_names = c("id", "n"))
...
sales_clean <- sales_raw |>
  mutate(is_brand = str_detect(id, "Brand"),
         brand = if_else(is_brand, id, NA)) |>
  fill(brand) |>
  filter(n != "n") |>
  select(brand, id, n) |>
