...
description: "End of lab on May 14"
categories: "Lab"
execute:
  eval: false
---
...
So, starting today in class, you'll learn to use a computer program (called R) to work with data.
...
Version control software like Git and GitHub is indispensable in modern data science practice, and learning how to use it is one of the things that distinguishes STA 199 from a similar course like STA 101.
...
```{r}
usethis::use_git_config(
  user.name = "Katie Solarz", 
  user.email = "katiesolarz@gmail.com"
)
```

::: callout-note
I used my personal email because that is the one I used to create my GitHub account.
**You must use the exact same email address that you used to create your GitHub account, even if it isn't your Duke email.**
:::
...
::: callout-note
Well, a more accurate statement would be that we will be reaching out to you to get things right during the first few days of class.
...
When you input the `usethis::use_git_config(...)` command into the console and hit enter/return, it will appear as if nothing happened.
To verify that everything worked, you can briefly switch over to the **Terminal** tab (should be to the right of **Console**) and type the commands `git config user.name` and `git config user.email`.
If all is well, these will return whatever text you originally provided.
If you notice a mistake or typo, you can just go back to the Console and rerun `usethis::use_git_config(...)` with modified inputs.
:::
...
A GitHub repository (repo) is a collection of files hosted on GitHub.
Repos are the main way we will distribute files to you during the semester.
When you copy the files in a repo to your local computing environment (your container, in this case), that's called "cloning".
So, let's clone our first repo:

-   Go to the repository for **lab-0** in the course GitHub organization: <https://github.com/sta199-su26/lab-0>.
...
-   Click on the green **CODE** button, select **Use SSH** (this might already be selected by default, and if it is, you'll see the text **Clone with SSH**).
    ...
    ![](img/lab-0/lab-0-repo.png){fig-alt="Clone with SSH button in GitHub repository page." width="600" fig-align="center"}
...
-   Click *Create Project*.
    ...
    ![](img/lab-0/lab-0-clone.png){fig-alt="Cloning a GitHub repository in RStudio." width="600" fig-align="center"}
...
-   Since this is the first time you are connecting to GitHub from this RStudio instance, you will be asked to confirm the authenticity of the host.
    ...
    ![](img/lab-0/authenticity-of-host.png){fig-alt="Authenticity of host pop-up window asking to confirm the authenticity of the host. Type 'yes' in the box and hit Enter." width="400" fig-align="center"}
...
Fill out the course "Getting to know you" [survey](https://canvas.duke.edu/courses/78571/quizzes/47322) on Canvas.

We will use the information collected in this survey for a variety of goals, including getting to know you as a person and your course goals and concerns, as well as pair you with project teammates with similar interests, be it personal or professional.
