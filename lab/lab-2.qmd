...
categories: Lab
description: "Due to Gradescope by Sun. May 24 11:59 PM"
draft: false
---
...
-   Go to the course organization at [github.com/sta199-su26](https://github.com/sta199-su26) organization on GitHub.
...
::: callout-important
If you encounter any issues with the above steps, flag your TA for help before proceeding.
:::
...
For this question, you will work with the following dataset called `df`.
The [`tibble()`](https://tibble.tidyverse.org/reference/tibble.html) function creates the data frame (the `tibble` object) `df` with three variables: `var_1`, `var_2`, and `var_3`.
The following code is already included in your lab document; press the green arrow to run the code chunk and load the `df` into your RStudio environment.
...
::: callout-tip
Check out the R Documentation for arrange() by typing “?a
rrange” into the console.
You should see the “Help” tab pop up in the bottom right pane of your RStudio environment.
Read through the “Description” section to confirm your understanding of the function.
:::
...
c\.
What does the following code chunk do?
Run it and analyze the result and articulate in words what each pipe does.
How many rows does the output have?
How many columns does the output have?
Is the output grouped?
If so, how many groups does it have?
...
How many possible combinations are there of the levels of `var_2` and `var_3`? You should use code to support your answers to each component of this question. 

::: callout-tip

Hint: The functions `unique()` and `count()` may be helpful here. 
...
d\.
In the following code cell, we add one more step to the data pipelines from the previous two parts. The new step uses slice_head(n = 1), which is designed to give the first row of the output. Run the code cell and analyze the result for each pipeline. You will notice the results are different. Why?
...
How many rows does the output have?
How many columns does the output have?
Is the output grouped? If so, how many groups does it have, and how many variables is it grouped on?
Articulate the difference(s) between summarize() and mutate().

::: callout-tip
Another hint: There are 2 key differences between `summarize()` and `mutate()` that your narrative should mention. Review the help files for each if you are stuck.
:::
...
Column names that are numbers are not considered "proper" in R; therefore, to work with / access them you'll need to wrap the column name in backticks.
:::

## Question 8

**In a single pipeline,** reshape (pivot) `life_expect` such that each row represents a country/year combination, with columns `country_name`,`year`, and `life_expectancy`; **save the result to a new data frame** Then, display the resulting data frame and state how many rows and columns it has.
...
-   The resulting data frame must be saved as something other than `life_expect` so you (1) can refer to this data frame later in your analysis and (2) do not overwrite `life_expect`. Use a short but informative name.

## Question 9

a\.
Create a vector called `countries_of_interest` which contains the names of between three and five countries you want to visualize the life expectancy for over the years.
For example, if these countries are France, Iceland, Denmark, and the United States, you can do this as follows:

```{r}
#| eval: false
countries_of_interest <- c("France", "United States", "Iceland", "Denmark")
...
b\.
**In a single pipeline**, filter your reshaped dataset from question 8 to include only the `countries_of_interest` from part (a) and **save the resulting data frame with a new name**.
**Do not overwrite the data frame** you're starting with.
Use a short but informative name.
**Then, in a new pipeline**, find the `distinct()` countries in the data frame you created to confirm that you've successfully filtered.
...
In a few sentences, describe the patterns you observe in the plot, particularly focusing on anything you find surprising or not surprising, based on your knowledge (or lack thereof) of these countries. Then, briefly explain why the pivot performed in question 9 was necessary for creating the this plot.

Requirements for the plot:

-   Data should be represented with **points** as well as **lines connecting the points** for each country (Note: lines should be directly connecting points, not a smooth trendline through them).
...
-   Plot should be **customized in at least one additional way** -- you could use a different than default color scale, or different than default theme (but NOT `theme_minimal()` or `theme_bw()` -- choose something more creative!), or some other customization.

## Question 11 - Workflow
