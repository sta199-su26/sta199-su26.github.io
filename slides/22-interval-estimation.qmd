...
format: 
  revealjs: 
    output-file: 22-interval-estimation-slides.html
...
todays_ae <- "ae-22-duke-forest-bootstrap"
...
df <- tibble(
  x = c(rnorm(300), rnorm(200, -4, 2))
)

ggplot(df, aes(x = x)) + 
  geom_histogram() + 
  geom_vline(xintercept = quantile(df$x, 0.08), color = "red", size = 2) + 
  geom_vline(xintercept = quantile(df$x, 0.87), color = "red", size = 2)
...
## The home stretch {.medium}

::: incremental
- Monday April 13: lecture on confidence intervals;
- Wednesday April 15: lecture on hypothesis testing;
- Thursday April 16: graded lab on statistics (last one!);
- Monday April 20: wrap-up lecture on inference;
- Wednesday April 22: lecture on who knows what (last one!);
- Wednesday April 22: turn in HW 7 (last one!);
- Friday May 1 : final exam @ 2pm.
:::


## On the horizon {.medium}

::: incremental
- Project grades posted this week
- Midterm 2 grades posted this week
- After Wednesday April 22: drop four days from attendance score
- After Lab 8 is graded: drop two lowest labs
- After HW 7 is graded: drop lowest HW
- 80% of your final course grade is *locked* going into the final
- I replace your lowest midterm score (the whole thing) with your final exam score, if it's better
:::

## Also

![](images/22/tom-jerry-rake.gif){fig-align="center"}
...
- In reality, we have *one* dataset, and it gives *one* set of results. How reliable are those results?
...
![](images/22/spongebob.jpg){fig-align="center"}
...
::: {.column width="50%"}
![](images/22/duke_forest_home.jpg){fig-alt="Home in Duke Forest"}
:::
:::::
