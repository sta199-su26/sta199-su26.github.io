...
::: callout-note
Answers are [here](/exams/midterm-1-answers.html).
:::
...
- In-class portion
    - Wednesday February 25 at 11:45 AM - 1:00 PM in Griffith Theater;
    - Worth 80% of your Midterm 1 grade;
    - All multiple choice;
...
- Take-home portion
    ...
    - Due Sunday March 1 at 11:59 PM;
    - Worth 20% of your Midterm 1 grade;
...
[Here](https://edstem.org/us/courses/90224/discussion/7626328) is our advice about how you should prioritize your studying. Below are 52 practice questions to get your wheels spinning.
...
i. What does it mean for a data analysis to be *reproducible*?
i. What is the difference between a *list* and a *vector* in `R`?
i. How does one pronounce "STA 199"?
i. What does the pipe operator `|>` do?
i. Inside `ggplot` or `geom_xxx`, what does the aesthetic mapping `aes` do?
i. What is the difference between `if_else` and `case_when`?
i. What is the difference between `filter` and `select`?
i. What does `group_by` do?
...
Match each scatterplot to the pair of histograms that displays the distributions of `x` and `y`. There is exactly one unique match for each.
...
Match the density plot to the boxplot that visualizes the distribution of the same data. There is exactly one unique match for each.
...
Below are ten examples of code that returns an error or a warning or otherwise behaves in a strange and unexpected way. In each case, identify the mistake and explain how it should be fixed.
...
You try to produce a plot and you get this error. Why?
...
You try again and you get this error. Why?
...
You want to color the points on your plot by whether or not the car's weight is above average. Why didn't this do the trick?
...
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g, shape = species, color = species)) + 
  geom_point() + 
  labs(
    title = "God I hate the penguins",
    x = "Stupid flipper length",
    y = "Bloody body mass",
    color = "Species"
  )
```
...
Earlier this year I surveyed you people about how many classes you have on Tuesdays, and you silly geese responded like this:
...
survey <- read_csv("data/survey-2025-02-06.csv") |>
  rename(
    tuesday = `How many classes do you have on Tuesdays?`,
    year = `What year are you?`
  ) |>
  select(tuesday, year) |>
  filter(
    tuesday %in% c("2", "1", "Two")
  )
```
...
survey |>
  mutate(
    tuesday = if_else(tuesday == "Two", 2, tuesday)
  )
```
...
**Question 31:** Your teammate asks you for help deciding which one to use in the final report for visualizing the relationship between performance rating and salary type.
...
**Question 32:** A friend with a keen eye points out that the number of observations in @fig-salary-performance-rating-1 seems lower than the total number of observations in `blizzard_salary`.
...
**Question 33:** Below are the proportions of performance ratings for hourly and salaried workers.
...
**Question 34:** Which of the following goes in blanks (1) and (2)?
...
**Question 35:** Which function or functions go into blank (3)?
